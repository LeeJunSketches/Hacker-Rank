...
function compareTriplets(a: number[], b: number[]): number[] {
    const compare = (rivalTriplets: number[]) => (acc: number, curr: number, index: number) => (curr > rivalTriplets[index] ? 1 : 0) + acc
    const alicePoints = a.reduce(compare(b) ,0);
    const bobPoints = b.reduce(compare(a), 0);
